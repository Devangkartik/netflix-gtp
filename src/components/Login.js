...
const Login = () =>{
 ...
 const name = useRef(null)
     ...
     const handleButtonClick = () =>{
         //validate the form data so we use utilities
         //checkValidData(email , password)

    //    console.log(email);  console.log(email.current.value)
    //    console.log(password);  console.log(password.current.value)
    ...
    return(  
        <div> 
            ...
            <form onSubmit={(e)=>{e.preventDefault()}} className='w-3/12  absolute p-12 bg-black my-36 mx-auto right-0 left-0 text-white bg-opacity-80'>
                ...
                {!isSignIn && <input ref={name} type='text' placeholder='Full name ' className='w-full  rounded-sm bg-gray-500 py-4 my-4  pl-4' />}
               ...
               <p className='font-bold text-red-600'>{errorMessage}</p>
                <button className='p-4 my-6 bg-red-700 w-full rounded-sm ' onClick={()=>{handleButtonClick()}}>{isSignIn ? "Sign In" : "Sign Up"}</button>
